...
polarized_3he_df = import_csv_with_pandas(file_path,10)
...
polarized_3he_df['Experiment'] = polarized_3he_df['Experiment'].replace(experiment_mapping)
dash_df = polarized_3he_df.replace(-1000.0, '-')

experiment_groups = {
    experiment: df for experiment, df in dash_df.groupby('Experiment')
}

cleaned_experiment_groups = {}

for experiment, df in experiment_groups.items():
    cleaned_df = df.loc[:, ~(df == '-').any()]
